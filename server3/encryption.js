...
const privateKeyLocation = "server3/wallet/" + (process.env.PRIVATE_KEY || "default")   //wallet안에 "default"라는 폴더가 있는지 확인해본다. 없으면 만든다.
...
function initWallet() {                         //암호화 파일 만드는 함수
    ...
    if (!fs.existsSync('server3/wallet/')) {            //wallet파일이없다면
        fs.mkdirSync("server3/wallet/")                 //파일만들고
    }
