...
function Test() {
    ...
    const bcMaker = async () => {
        ...
        await axios.post(`http://localhost:3002/mineBlock`, { data: [data] })
            .then(req => alert(req.data))
    }

    const connect = async () => {
        await axios.get(`http://localhost:3002/Blocks`)
            .then(req => setChainBlocks(req.data))
    }

    const address = async () => {
        await axios.get(`http://localhost:3002/address`)
    ...
    const getpeers = async () => {
        axios.get(`http://localhost:3002/peers`)
    ...
    const addPeers = async () => {
        ...
        await axios.post(`http://localhost:3002/addPeers`, { peers: [`ws://localhost:${P}`] })
            .then(req => alert(req.data))
    }
    ...
    useInterval(() => {
        ...
        setIsRunning(false)
        console.log("데이터전송")
        axios.post(`http://localhost:3002/mineBlock`, { data: [data] })
            .then((req) => {
    ...
    return (
        <div >
            <Row>
                <Col span={24}>   <h1>3002포트 WS6001입니다.</h1></Col>
            </Row>
            <br />
            ...
            <Button style={{ marginLeft: 30, }} type="dashed" onClick={() => { alert("채굴을 시작합니당."); setIsRunning(true), setOk(true) }}>채굴</Button>
            <Button style={{ marginLeft: 30, }} type="dashed" onClick={() => { alert("채굴을 중지합니당."); setOk(false); }}>중지</Button>
